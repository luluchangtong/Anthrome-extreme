...
fig,axs = plt.subplots(3,1, sharex = True, sharey = True, figsize = (20,15))
...
index_x = index_x - (total_width - width) / 2
for i in range(3):
    
        axs[i].bar(index_x,mean_value.iloc[:,i*4+0],color = 'powderblue',alpha = 0.8,label = '80th',align = 'center',width = width)
        axs[i].bar(index_x+width,mean_value.iloc[:,i*4+1],color = 'deepskyblue',alpha = 0.8,label = '90th',align = 'center',width = width)
        axs[i].bar(index_x+width*2,mean_value.iloc[:,i*4+2],color = 'royalblue',alpha = 0.8,label = '95th',align = 'center', width =width)
        axs[i].bar(index_x+width*3,mean_value.iloc[:,i*4+3],color = 'darkblue',alpha = 0.8,label = '99th',align = 'center', width = width)
...
axs[0].legend(['80th','90th','95th','99th'],loc = 'upper right',fontsize =19,prop=dict(weight='bold'))
fig.tight_layout()

plt.savefig(r'C:\Users\54021\OneDrive\桌面\桌面\meeting\figure\figures_new\sensitivity_biome', dpi = 500)