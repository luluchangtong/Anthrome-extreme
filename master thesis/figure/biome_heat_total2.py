...
for i in range(0,5):
    ...
    for j in range(3):
        ...
        axs[i,j].fill_between(y1 = 0,y2 =ssp[j][ssp[j].columns[i+15]],color = 'red',alpha = 1,x = ssp126.index)
        axs[i,j].fill_between(y1 = 0, y2 =ssp_total[j][ssp_total[j].columns[i+15]],color='tab:blue',alpha = 0.5,x = ssp126.index)
        axs[i,j].set_title(SSP[j],fontweight = 'bold', loc = 'left',y = 0.90,x=0.05, fontsize = 10)

axs[4,1].set_xlabel('Year', fontweight = 'bold')
fig.savefig(r'C:\Users\54021\OneDrive\桌面\桌面\meeting\figure\figures_new\biome_fre4',dpi = 800)
