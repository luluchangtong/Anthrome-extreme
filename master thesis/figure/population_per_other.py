...
All_files = os.listdir(r'D:\UUstudy\assignment\Master thesis\Dataprocess\human_analysis\total_population\GCMs_mean')
DATA = list(filter(lambda x: re.search('30',x) != None, All_files))

files_path = [os.path.join(r'D:\UUstudy\assignment\Master thesis\Dataprocess\human_analysis\total_population\GCMs_mean',x) for x in DATA]
...
All_files2 = os.listdir(r'D:\UUstudy\assignment\Master thesis\Dataprocess\human_analysis\population_%\GCMs_mean')
DATA2 = list(filter(lambda x: re.search('30',x) != None, All_files2))
files_path2 = [os.path.join(r'D:\UUstudy\assignment\Master thesis\Dataprocess\human_analysis\population_%\GCMs_mean',x) for x in DATA2]
...
All_files3 = os.listdir(r'D:\UUstudy\assignment\Master thesis\Dataprocess\human_analysis\population_%_10d\GCMs_mean')
DATA3 = list(filter(lambda x: re.search('30',x) != None, All_files3))
files_path3 = [os.path.join(r'D:\UUstudy\assignment\Master thesis\Dataprocess\human_analysis\population_%_10d\GCMs_mean',x) for x in DATA3]
...
fig,axs = plt.subplots(1,4, sharex = False, sharey = True, figsize = (18,5))
ws = 0.1
fig.subplots_adjust(wspace=ws)

axs[0].set_title('Pastoral villages',fontweight = 'bold', loc = 'center',y = 1.02, x = -0.10,fontsize = 15)
#sub figure 1
...
y2 = [np.mean([pop_ssp126_30.iloc[0,5],pop_ssp370_30.iloc[0,5],pop_ssp585_30.iloc[0,5]]),pop_ssp126_30.iloc[85,5],pop_ssp370_30.iloc[85,5],pop_ssp585_30.iloc[85,5]]
y1 = [np.mean([pop_tol_ssp126_30.iloc[0,5],pop_tol_ssp370_30.iloc[0,5],pop_tol_ssp585_30.iloc[0,5]]), pop_tol_ssp126_30.iloc[85,5], pop_tol_ssp370_30.iloc[85,5], pop_tol_ssp585_30.iloc[85,5]]
y3 = [np.mean([pop_10_ssp126_30.iloc[0,5], pop_10_ssp370_30.iloc[0,5],pop_10_ssp585_30.iloc[0,5]]),pop_10_ssp126_30.iloc[85,5],pop_10_ssp370_30.iloc[85,5],pop_10_ssp585_30.iloc[85,5]]
...
for a,b,c in zip(x,y2,y1):
    axs[3].text(a,b+0.05, str('%.1f' % (b/c*100))+'%', va = 'bottom', fontsize = 7,horizontalalignment='left')
    
for a,b,c in zip(x,y3,y1):
    axs[3].text(a,b+0.05, str('%.1f' % (b/c*100))+'%', va = 'top', fontsize = 7,horizontalalignment='left')
    
plt.savefig(r'C:\Users\54021\Desktop\meeting\figure\pop_Pastoral villages',dpi = 500)
