...
def get_all_abs_path(source_dir):
    ...
    return path_list
files_list = get_all_abs_path(r'C:\UUstudy\assignment\Master thesis\Dataprocess\Frequency\human\GCM_mean')
files_list = sorted(files_list)

threshold = ['25','26','28','30','33']
...
for i in range(5):
    for j in range(3):
        ...
        x, y = mp(lon,lat)
        mask_ocean = maskoceans(lon,lat,SSP[j*5+i],resolution = 'c')
...
for j in range(5):
    axs[j,0].set_ylabel(f'WBGT > {threshold[j]}°C',fontsize = 5)
...
plt.savefig(r'C:\Users\54021\OneDrive\桌面\桌面\meeting\figure\figures_new\human\spatial_uncertainty',dpi = 1200,bbox_inches = 'tight')
