...
for ssp,ssp_an in zip(SSP, SSP_an):
    ...
    for tem_sel in tem:
        ...
        Human_percentage_30 = pd.DataFrame(index = list(anthrome_cls.index), columns=list(range(2015,2101)))
        ...
        for i in range(2015,2101):
            ...
            for j in anthrome_cls.index:
                ...
                heat_select_30 = ((fre>30) & grid_sel)
                
                # calculate total exposure 
                
                
                Human_percentage_30[i][j] = np.sum(pop[heat_select_30])
        ...
        file_abs_name1 = 'population_%_30d_' + fre_path_split
        file_excel1 = file_abs_name1.replace('.nc','.xlsx')
        file_path1 = '/mnt/data/luyin/Dataprocess/human_analysis/population_%_30d/GCMs_mean/' + file_excel1
        Human_percentage_30.to_excel(file_path1,index = True)
